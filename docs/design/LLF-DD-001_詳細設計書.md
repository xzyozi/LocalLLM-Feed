...
| `params_b`     | モデル名の `NB` 表記、safetensors/config 情報 | 例 `35B` → `35.0`。MoE 表記（例 `8x7B`）は各エキスパートのパラメータ数（`7.0`）を名目値とする（VRAM 概算が単一代表値を前提とするため、総実効パラメータは採らない）。抽出不能時は名前からの推定、無理なら除外候補。 |
